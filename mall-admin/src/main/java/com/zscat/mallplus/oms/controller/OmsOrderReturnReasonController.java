...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有退货原因表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存退货原因表")
...
@SysLog(MODULE = "sys", REMARK = "更新退货原因表")
...
@SysLog(MODULE = "sys", REMARK = "删除退货原因表")
@ApiOperation("删除退货原因表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("退货原因表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给退货原因表分配退货原因表")
@ApiOperation("查询退货原因表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("退货原因表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("退货原因表id");
        }
