...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有订单表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存订单表")
@ApiOperation("保存订单表")
...
@SysLog(MODULE = "sys", REMARK = "更新订单表")
@ApiOperation("更新订单表")
...
@SysLog(MODULE = "sys", REMARK = "删除订单表")
@ApiOperation("删除订单表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("订单表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给订单表分配订单表")
@ApiOperation("查询订单表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("订单表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("订单表id");
        }
