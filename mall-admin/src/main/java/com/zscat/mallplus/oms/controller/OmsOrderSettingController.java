...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有订单设置表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存订单设置表")
...
@SysLog(MODULE = "sys", REMARK = "更新订单设置表")
...
@SysLog(MODULE = "sys", REMARK = "删除订单设置表")
@ApiOperation("删除订单设置表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("订单设置表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给订单设置表分配订单设置表")
@ApiOperation("查询订单设置表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("订单设置表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("订单设置表id");
        }
