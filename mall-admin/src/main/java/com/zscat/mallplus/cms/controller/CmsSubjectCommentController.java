...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有专题评论表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存专题评论表")
...
@SysLog(MODULE = "sys", REMARK = "更新专题评论表")
...
@SysLog(MODULE = "sys", REMARK = "删除专题评论表")
@ApiOperation("删除专题评论表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("专题评论表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给专题评论表分配专题评论表")
@ApiOperation("查询专题评论表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("专题评论表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("专题评论表id");
        }
