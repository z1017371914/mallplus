...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有用户举报表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存用户举报表")
...
@SysLog(MODULE = "sys", REMARK = "更新用户举报表")
...
@SysLog(MODULE = "sys", REMARK = "删除用户举报表")
@ApiOperation("删除用户举报表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("用户举报表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给用户举报表分配用户举报表")
@ApiOperation("查询用户举报表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("用户举报表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("用户举报表id");
        }
