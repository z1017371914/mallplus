...
@SysLog(MODULE = "sys", REMARK = "根据条件查询所有专题分类表列表")
...
@SysLog(MODULE = "sys", REMARK = "保存专题分类表")
...
@SysLog(MODULE = "sys", REMARK = "更新专题分类表")
...
@SysLog(MODULE = "sys", REMARK = "删除专题分类表")
@ApiOperation("删除专题分类表")
@DeleteMapping(value = "/delete/{id}")
@PreAuthorize("hasAuthority('sys:role:delete')")
public Object deleteRole(@ApiParam("专题分类表id") @PathVariable Long id){
        try{
...
@SysLog(MODULE = "sys", REMARK = "给专题分类表分配专题分类表")
@ApiOperation("查询专题分类表明细")
@GetMapping(value = "/{id}")
@PreAuthorize("hasAuthority('sys:role:read')")
public Object getRoleById(@ApiParam("专题分类表id") @PathVariable Long id){
        try{
        if(ValidatorUtils.empty(id)){
        return new CommonResult().paramFailed("专题分类表id");
        }
